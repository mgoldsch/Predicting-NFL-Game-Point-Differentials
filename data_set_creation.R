...
for(t in teams[1]){
  ...
  #rollmean score
  score_avg <- zoo::rollmean(ifelse(rollmean_temp$home_team == t, rollmean_temp$home_score, rollmean_temp$away_score), k = 5, fill = NA) #calculate rollmean of score for team
  rollmean_temp$home_score_5w_avg <- ifelse(rollmean_temp$home_team == t, score_avg, NA) #populate the rollmean score into home column if the team is home
  rollmean_temp$away_score_5w_avg <- ifelse(rollmean_temp$away_team == t, score_avg, NA) #populate the rollmean score into away column if the team is away
  
  #rollmean point_differential
  point_diff_avg <- zoo::rollmean(ifelse(rollmean_temp$home_team == t, rollmean_temp$point_differential, rollmean_temp$point_differential * -1), k = 5, fill = NA)
  rollmean_temp$home_point_diff_5w_avg <- ifelse(rollmean_temp$home_team == t, point_diff_avg, NA) #populate the rollmean point_diff_avg into home column if the team is home
  rollmean_temp$away_point_diff_5w_avg <- ifelse(rollmean_temp$away_team == t, point_diff_avg, NA) #populate the rollmean point_diff_avg into away column if the team is away
  
  #rollmean .....
  
  #join with nfl_rollmeans
  
}

#join nfl_rollmeans with nfl_data_set
