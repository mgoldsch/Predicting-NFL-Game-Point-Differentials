...
game_team_yards <- pbp_db %>%
  dplyr::filter(!is.na(posteam) & posteam != "") %>%
  dplyr::group_by(game_id, posteam, fixed_drive) %>%
  dplyr::summarise(pass_yards = sum(passing_yards, na.rm = TRUE), rush_yards = sum(rushing_yards, na.rm = TRUE)) %>%
  dplyr::ungroup() %>%
  dplyr::group_by(game_id, posteam) %>%
  dplyr::summarise(off_passing_yards = sum(pass_yards, na.rm = TRUE), off_rushing_yards = sum(rush_yards, na.rm = TRUE)) %>%
  dplyr::select(game_id, posteam, off_passing_yards, off_rushing_yards) %>% 
  dplyr::collect()
game_team_subgroup_yards
