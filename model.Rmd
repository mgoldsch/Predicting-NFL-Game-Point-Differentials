...
model_big_data_use <- nfl_data_set %>% 
  ...
  dplyr::select(point_differential, season, week, home_coach, away_coach, roof, surface, start_time,
                ...
                off_rushing_yards_qtr_3_away_5w_avg, off_rushing_yards_qtr_4_away_5w_avg, #rmse 11.008 with only these, rmse 11.178 when including these...likely due to model not being tuned and more vars cause defaults to be less good?
...
```{r}
train_big <- filter(model_big_data_use, season %in% seq(1999, 2015))
...
```{r}
par(pty = "s")
plot(big_model_preds, test_big$point_differential, asp = 1) #flip axes back
abline(a = 0, b = 1)
```
